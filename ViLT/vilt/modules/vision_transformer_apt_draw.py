...
class approximate_prompt(nn.Module):
    def __init__(
        self,
        num_heads,
        dim_feature,
        ...
        self.hidden = hid_feature

        self.mm = nn.Sequential(
            nn.Linear(dim_feature, self.hidden, bias=False),
            nn.Linear(self.hidden, dim_feature, bias=False),
        )
        nn.init.normal_(self.mm[0].weight.data, mean=0, std=1.00)
        nn.init.constant_(self.mm[1].weight.data, 0)

        # self.mm = nn.Linear(dim_feature, dim_feature, bias=False)
        # nn.init.normal_(self.mm.weight.data, mean=0, std=1.00)

        self.mm_tokens = nn.Parameter(torch.FloatTensor(self.tot_memory, dim_feature))
        nn.init.normal_(self.mm_tokens.data, mean=0, std=0.02)
    ...
    def forward(self, x, res):
        ...
        attn = res @ (mm_tokens + self.mm(mm_tokens)).t()
        # attn = res @ (mm_tokens).t()
        attn = self.activation(attn)
        attn = self.attn_drop(attn)

        e = attn @ mm_tokens
        
        # weights = attn.max(dim=-1, keepdim=True)[0].clone().detach() * self.weight.exp()
        # weights = weights / (weights + 1)

        # if self.training:
        #     weights = (torch.softmax(attn, dim=-1) * attn).sum(dim=-1, keepdim=True) * self.weight
        # else:
        #     weights = attn.max(dim=-1, keepdim=True)[0] * self.weight

        weights = attn.max(dim=-1, keepdim=True)[0].clone().detach()

        x = x + e * weights
        x = x * self.weight.exp()

        mm_tokens_shape = self.mm_tokens.data.pow(2).sum(-1).unsqueeze(dim=0).unsqueeze(dim=0)
        attn = attn * mm_tokens_shape
...
class Mlp(nn.Module):
    def __init__(
        ...
        # self.APT = approximate_prompt(12, in_features, 128, 0.1)
...
class Attention(nn.Module):
    def __init__(
        ...
        self.APT = approximate_prompt(num_heads, dim, 4, 200, attn_drop)
    ...
    def forward(self, x, mask=None):
        ...
        q, k, v = (
            ...
            qkv[1],
            qkv[2],
        )  # make torchscript happy (cannot use tensor as tuple) B x H x N x C
        ...
        x = (attn @ v) # B x H x N C

        x = x.transpose(1, 2).reshape(B, N, C)

        x = self.proj(x)
        x, attn_ = self.APT(x, res)

        x = self.proj_drop(x)

        attn = attn.mean(1) # B x N x N
        res_shape = res.pow(2).sum(-1).sqrt().unsqueeze(dim=-1).transpose(1, 2) # B x 1 x N
        attn = attn * res_shape

        attn = torch.cat([attn, attn_], dim=-1)
        attn = attn.softmax(dim=-1)
