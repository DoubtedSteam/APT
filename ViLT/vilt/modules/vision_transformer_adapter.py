...
class Block(nn.Module):
    def __init__(
        ...
        self.adapter0 = Adapter(768)
        self.adapter1 = Adapter(768)

    def forward(self, x, mask=None):
        _x, attn = self.attn(self.norm1(x), mask=mask)
        x = x + self.drop_path(self.adapter0(_x))
        x = x + self.drop_path(self.adapter1(self.mlp(self.norm2(x))))
        return x, attn
