...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, prompt_num=0):
        B, N, C = x.shape
        res = x
        qkv = (
            self.qkv(x)
            .reshape(B, N, 3, self.num_heads, C // self.num_heads)
            .permute(2, 0, 3, 1, 4) # 3xBxHxNxD
        )
        q, k, v = (
            qkv[0],
            qkv[1],
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N-prompt_num, C)
        x = self.proj(x)
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None, prompt_num=0):
        _x, attn = self.attn(self.norm1(x), mask=mask, prompt_num=prompt_num)
        x = x[:, :-prompt_num, :] + self.drop_path(_x)
