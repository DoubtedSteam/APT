...
import vilt.modules.vision_transformer_baseline as vit
...
class ViLTransformerSS(pl.LightningModule):
    def __init__(self, 
                 config, 
                 trainable=["classifier", "pooler", "token_type_embeddings", "rank_output"] + ["prompt"]
                ):
        super().__init__()
        ...
        if self.hparams.config["load_path"] == "":
            self.transformer = getattr(vit, self.hparams.config["vit"])(
                pretrained=True, config=self.hparams.config, PS_list=PS_list
            )
        else:
            self.transformer = getattr(vit, self.hparams.config["vit"])(
                pretrained=False, config=self.hparams.config, PS_list=PS_list
            )
        ...
        self.prompt_num = 200
        self.prompt = nn.Parameter(torch.zeros(1, 1, self.prompt_num, 768))
    ...
    def infer(
        ...
        bsz = co_masks.shape[0]
        prompt_mask = torch.ones(bsz, self.prompt_num, device=co_masks.device)
        co_masks = torch.cat([co_masks, prompt_mask], dim=1)

        x = co_embeds

        ex_co_maks = torch.bmm(co_masks.unsqueeze(2), co_masks.unsqueeze(1))
        ...
        # attn_matrix = []
        x = torch.cat([x, self.prompt[0].repeat(bsz, 1, 1)], dim=1)
        ...
        #     cv2.imwrite('visual_shallow/{}.png'.format(i), attn)
