...
class lora(nn.Module):
    ...
    def forward(self, res, x):
        ...
        x = x.permute(0, 2, 1, 3).reshape(B, N, H*C)
        x = x + res
        x = x.reshape(B, N, H, C).permute(0, 2, 1, 3)
...
class Attention(nn.Module):
    def __init__(
        ...
        self.q_lora = lora(dim, 204)
        self.v_lora = lora(dim, 204)
    ...
    def forward(self, x, mask=None):
        ...
        qkv = (
            ...
            .permute(2, 0, 3, 1, 4) # 3 x B x H x N x C
        )
        q, k, v = (
            qkv[0],
            qkv[1],
