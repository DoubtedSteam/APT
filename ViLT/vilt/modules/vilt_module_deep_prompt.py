...
import vilt.modules.vision_transformer_deep_prompt as vit
...
class ViLTransformerSS(pl.LightningModule):
    def __init__(self, 
        ...
        self.prompt_num = 200
        self.prompt = nn.Parameter(torch.zeros(12, 1, self.prompt_num, 768))
    ...
    def infer(
        ...
        for i, blk in enumerate(self.transformer.blocks):
            x = torch.cat([x, self.prompt[i].repeat(bsz, 1, 1)], dim=1)
            x, _attn = blk(x, mask=co_masks, prompt_num=self.prompt_num)
