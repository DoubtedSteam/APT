...
from .bert_model_LST import BertCrossLayer, BertAttention
...
class Backbone(nn.Module):
    def __init__(self, modules):
        ...
        self.side_block_downsampler_text_layers = modules[9]
        self.side_block_downsampler_image_layers = modules[10]
        self.side_block_cross_modal_text_layers = modules[11]
        self.side_block_cross_modal_image_layers = modules[12]
        self.side_block_upsampler_text = modules[13]
        self.side_block_upsampler_image = modules[14]
    ...
    def forward(self, text_embed, img):
        ...
        x, y = text_embeds, image_embeds
        side_x, side_y = self.side_block_downsampler_text_layers[-1](x.clone().detach()), self.side_block_downsampler_image_layers[-1](y.clone().detach())
        for idx, (text_layer, image_layer) in enumerate(zip(self.cross_modal_text_layers, self.cross_modal_image_layers)):
            x1 = text_layer(x, y, extend_text_masks, extend_image_masks)
            y1 = image_layer(y, x, extend_image_masks, extend_text_masks)

            side_x = side_x + self.side_block_downsampler_text_layers[idx](x1[0].clone().detach())
            side_y = side_y + self.side_block_downsampler_image_layers[idx](y1[0].clone().detach())

            x2 = self.side_block_cross_modal_text_layers[idx](side_x, side_y, extend_text_masks, extend_image_masks)
            y2 = self.side_block_cross_modal_image_layers[idx](side_y, side_x, extend_image_masks, extend_text_masks)

            x, y, side_x, side_y = x1[0], y1[0], x2[0], y2[0]

        x = self.side_block_upsampler_text(side_x)
        y = self.side_block_upsampler_image(side_y)
...
class METERTransformerSS(pl.LightningModule):
    def __init__(self, 
                 config,
                 trainable=["classifier", "pooler", "token_type_embeddings", "rank_output", "side_block"]
                ):
        super().__init__()
        ...
        if 'roberta' in config['tokenizer']:
            bert_config = RobertaConfig(
                vocab_size=config["vocab_size"],
                hidden_size=config["hidden_size"] // self.down_ratio,
                num_hidden_layers=config["num_layers"],
                num_attention_heads=config["num_heads"],
                intermediate_size=config["hidden_size"] * config["mlp_ratio"],
                max_position_embeddings=config["max_text_len"],
        ...
        self.side_block_cross_modal_image_layers = nn.ModuleList([BertCrossLayer(bert_config) for _ in range(config['num_top_layer'])])
        self.side_block_cross_modal_image_layers.apply(objectives.init_weights)
        self.side_block_cross_modal_text_layers = nn.ModuleList([BertCrossLayer(bert_config) for _ in range(config['num_top_layer'])])
        self.side_block_cross_modal_text_layers.apply(objectives.init_weights)

        self.side_block_downsampler_image_layers = nn.ModuleList([nn.Linear(config["hidden_size"], config["hidden_size"] // self.down_ratio) for _ in range(config['num_top_layer'] + 1)])
        self.side_block_downsampler_text_layers = nn.ModuleList([nn.Linear(config["hidden_size"], config["hidden_size"] // self.down_ratio) for _ in range(config['num_top_layer'] + 1)])

        self.side_block_upsampler_image = nn.Linear(config["hidden_size"] // self.down_ratio, config["hidden_size"])
        self.side_block_upsampler_image.apply(objectives.init_weights)
        self.side_block_upsampler_text = nn.Linear(config["hidden_size"] // self.down_ratio, config["hidden_size"])
        self.side_block_upsampler_text.apply(objectives.init_weights)
        ...
        extra_param = sum(p.numel() for n, p in self.named_parameters() if "side_block" in n)
        ...
        model = Backbone([
            ...
            self.side_block_downsampler_text_layers,
            self.side_block_downsampler_image_layers,
            self.side_block_cross_modal_text_layers,
            self.side_block_cross_modal_image_layers,
            self.side_block_upsampler_text,
            self.side_block_upsampler_image
        ])
    ...
    def infer(
        ...
        for idx, (text_layer, image_layer) in enumerate(zip(self.cross_modal_text_layers, self.cross_modal_image_layers)):
            ...
            side_x = side_x + self.side_block_downsampler_text_layers[idx](x.clone().detach())
            side_y = side_y + self.side_block_downsampler_image_layers[idx](y.clone().detach())
