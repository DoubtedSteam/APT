...
from .bert_model_prompt import BertCrossLayer, BertAttention
...
class METERTransformerSS(pl.LightningModule):
    def __init__(self, 
                 config,
                 trainable=["classifier", "pooler", "token_type_embeddings", "rank_output"] \
                         + ["prompt"]
                ):
        super().__init__()
        ...
        self.prompt_num = 200
        self.prompts = nn.ParameterList([nn.Parameter(torch.zeros(6, 1, self.prompt_num, 768)) for _ in range(2)])
        ...
        extra_param = sum(p.numel() for n, p in self.named_parameters() if "prompt" in n)
    ...
    def infer(
        ...
        prompt_mask = torch.ones(bsz, self.prompt_num, device=extend_image_masks.device)

        text_masks = torch.cat([prompt_mask, text_masks], dim=1)
        extend_text_prompt_masks = self.text_transformer.get_extended_attention_mask(text_masks, text_masks.shape, device)
        image_masks = torch.cat([prompt_mask, image_masks], dim=1)
        extend_image_prompt_masks = self.text_transformer.get_extended_attention_mask(image_masks, image_masks.shape, device)

        x, y = text_embeds, image_embeds
        for i, [text_layer, image_layer] in enumerate(zip(self.cross_modal_text_layers, self.cross_modal_image_layers)):

            # x_p = torch.cat([self.prompts[0][i].repeat(bsz, 1, 1), x], dim=1)
            # y_p = torch.cat([self.prompts[1][i].repeat(bsz, 1, 1), y], dim=1)
            x_t = torch.cat([self.prompts[0][i].repeat(bsz, 1, 1), x], dim=1)
            y_t = torch.cat([self.prompts[1][i].repeat(bsz, 1, 1), y], dim=1)

            x1 = text_layer(x, y_t, extend_text_masks, extend_image_prompt_masks)
            y1 = image_layer(y, x_t, extend_image_masks, extend_text_prompt_masks)
            x, y = x1[0], y1[0]
