...
class METERTransformerSS(pl.LightningModule):
    def __init__(self, 
        ...
        self.prompt_num = 200
        self.prompts = nn.ParameterList([nn.Parameter(torch.zeros(1, 1, self.prompt_num, 768)) for _ in range(2)])
    ...
    def infer(
        ...
        x = torch.cat([self.prompts[0][0].repeat(bsz, 1, 1), x], dim=1)
        y = torch.cat([self.prompts[1][0].repeat(bsz, 1, 1), y], dim=1)
        for i, [text_layer, image_layer] in enumerate(zip(self.cross_modal_text_layers, self.cross_modal_image_layers)):
            x1 = text_layer(x, y, extend_text_prompt_masks, extend_image_prompt_masks)
            y1 = image_layer(y, x, extend_image_prompt_masks, extend_text_prompt_masks)
            x, y = x1[0], y1[0]
